...
function login() {
   ...
   if (email === '' || password === '') {
        ...
        // Ovde može da ide prava provera kredencijala (npr. API poziv)

        // Za sada simulacija uspešnog logina:
        localStorage.setItem('ulogovan', 'true');      // Pamti da je korisnik ulogovan
        localStorage.setItem('userEmail', email);       // Pamti email korisnika (može i drugo)
        
        alert('Login Successful (fake)');
        location.reload();  // Osveži stranicu da se meni update-uje
    }
}
